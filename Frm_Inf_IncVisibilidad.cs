...
namespace T3
{
    public partial class Frm_Inf_IncVisibilidad : Form
    {
        ...
        {
            string _Str_Cadena = _Str_Cadena = "SELECT DISTINCT CONVERT(VARCHAR,cmesventas)+'-'+CONVERT(VARCHAR,canoventas),CONVERT(VARCHAR,cmesventas)+'-'+CONVERT(VARCHAR,canoventas) FROM TCONFIGINCVTAS WHERE ccompany='" + Frm_Padre._Str_Comp + "' AND (cejec1generado <> 0 OR cejec2generado <> 0)";
        ...
        private void _Mtd_CargarGrupos()
        {
            ...
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TGRUPOIV.cidgrupincentivar, TGRUPOIV.cdescripcion " +
                                 ...
                                 "TCALINCVIS ON TGRUPOIV.cidgrupincentivar = TCALINCVIS.cidgrupincentivar AND " +
                                 "TGRUPOIV.cgroupcomp = TCALINCVIS.cgroupcomp AND TGRUPOIV.ccompany = TCALINCVIS.ccompany " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCVIS.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCVIS.cyearacco = '" + _Str_Valores[1] + "') AND (TCALINCVIS.cmontacco = '" + _Str_Valores[0] + "') " +
        ...
        {
            string[] _Str_Valores = _Mtd_ExtraerMesAno(_Cmb_Mes.SelectedValue.ToString());
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TCALINCVIS.cvendedor, TCALINCVIS.cvendedor+' - '+ TUSER.cname, TUSER.cname " +
                                 "FROM TGRUPOIV INNER JOIN " +
                                 "TCALINCVIS ON TGRUPOIV.cidgrupincentivar = TCALINCVIS.cidgrupincentivar AND " +
                                 "TGRUPOIV.cgroupcomp = TCALINCVIS.cgroupcomp AND TGRUPOIV.ccompany = TCALINCVIS.ccompany INNER JOIN " +
                                 "TUSER ON TCALINCVIS.cvendedor = TUSER.cuser " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCVIS.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCVIS.cyearacco = '" + _Str_Valores[1] + "') AND (TCALINCVIS.cmontacco = '" + _Str_Valores[0] + "') AND (TGRUPOIV.cidgrupincentivar = '" + Convert.ToString(_Cmb_Grupo.SelectedValue).Trim() + "') " +
        ...
        private void _Mtd_Busqueda()
        {
            ...
            parm[4] = new ReportParameter("CTIPOGENERACION", Convert.ToString(_Cmb_Generacion.SelectedValue).Trim());
            parm[5] = new ReportParameter("CNOMBEMP", CLASES._Cls_Varios_Metodos._Mtd_NombComp(Frm_Padre._Str_Comp));
            parm[6] = new ReportParameter("CUSER", _Str_Usuario);
            _Rpt_Report.ServerReport.SetParameters(parm);
            _Rpt_Report.ServerReport.Refresh();
            _Rpt_Report.RefreshReport();
        }
        private void Frm_Inf_IncVisibilidad_Load(object sender, EventArgs e)
        {
        ...
        {
            _Rpt_Report.ServerReport.DisplayName = CLASES._Cls_Varios_Metodos._Mtd_NombreReportesExportacion("RPT_INC_VISIBILIDAD");
