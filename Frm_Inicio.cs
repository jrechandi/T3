...
namespace T3
{
    public partial class Frm_Inicio : Form
    {
        ...
        void _Txt_Clave_GotFocus(object sender, EventArgs e)
        {
            ...
            { MessageBox.Show("Usuario sin acceso otorgado,", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Exclamation); _Bol_Acceso = false; }
            if (_Bol_Acceso)
            {
        ...
        private void _Bt_Ok_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("En estos momentos se están realizando cambios en el sistema. Por favor intente mas tarde.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Application.Exit();
            }
            else
            {
                ...
                if (_Mtd_VerificarClave(_Txt_Clave.Text.Trim()))
                {
                    ...
                    if (_DsD.Tables[0].Rows.Count > 0)
                    {
                        if (_Mtd_UsuarioNoTieneClaveReseteada(_Txt_User.Text.Trim()))
                        {
                            if (_Mtd_UsuarioNoEstaInactivado(_Txt_User.Text.Trim()))
                            {
                                ...
                                if (false)
                                {
                                    //MessageBox.Show("Existe un problema con el calendario de cierre, por lo tanto no podrá trabajar en el sistema, por favor infórmelo a la corporativa.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    //Application.Exit();
                                }
                                else
                                {
                                    if (!_Mtd_VerificarLoginRestric(_Txt_User.Text.Trim()))
                                    {
                                        if (_Bol_Advertencia)
                                        {
                                            string _Str_Mensaje = "Se ha detectado que el USUARIO y la CONTRASEÑA que usted acaba de ingresar están siendo utilizados en dos o más equipos al mismo tiempo. En un futuro cercano T3 no le permitirá ingresar de esta manera, por lo que le recomendamos lo siguiente: \n\n*Si usted es un usuario registrado del sistema, solicite a su supervisor un reseteo de clave. No comparta su clave con ningún otro usuario. \n*Si usted aún no está registrado en el sistema, solicite a su supervisor un USUARIO y CONTRASEÑA propios. \nRecuerde que no debe compartir su contraseña con ningún otro usuario. Si lo hace, entonces usted está afectando negativamente los procesos.";
                                            MessageBox.Show(_Str_Mensaje, "Información", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                        }
                                        //Cursor = Cursors.WaitCursor;
                                    ...
                                    {
                                        string _Str_Mensaje = "Se ha detectado que el USUARIO y la CONTRASEÑA que usted acaba de ingresar están siendo utilizados en dos o más equipos al mismo tiempo.\n\nEn caso de que el sistema haya sido cerrado de manera abrupta en su equipo, por favor espere 5 minutos e intente de nuevo.";
                                        MessageBox.Show(_Str_Mensaje, "Información", MessageBoxButtons.OK, MessageBoxIcon.Stop);
                                    }
                                }
                            }
                            else
                            {
                                this.Activated -= new EventHandler(Frm_Inicio_Activated);
                                MessageBox.Show("Disculpe, este usuario ha sido inhabilitado. Por favor contacto a su supervisor.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        MessageBox.Show("El usuario ha sido eliminado.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
                    this.Activated -= new EventHandler(Frm_Inicio_Activated);
                    MessageBox.Show("La clave es incorrecta", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    _Txt_Clave.Select(0, _Txt_Clave.Text.Length);
                    _Txt_Clave.Focus();
                    this.Activated += new EventHandler(Frm_Inicio_Activated);
                }
            ...
            if (_Str_NuevoPassword.Length >= 4)
            {
                if (_Mtd_NuevoPasswordEsDiferenteDeProvisional(_Str_Login, _Str_NuevoPassword))
                {
                    ...
                    try
                    {
                        ...
                        Program._MyClsCnn._mtd_conexion._Mtd_EjecutarSentencia(_Str_SQL);
                        //-----------------------
                        MessageBox.Show("La nueva clave ha sido establecida satisfactoriamente.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.Height = _Int_FormHeight;
                        _Cmb_Companies.Enabled = true;
                        _Txt_User.Enabled = true;
        ...
        private void Frm_Inicio_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (_Bol_Mensaje)
            {
                if (CLASES._Cls_Conexion._Bol_ConexionRemota) // si T3Win está configurado como 'remoto'...
                {
                    // intenta descargar de un repositorio local
                    _Str_DireccionActT3 = _Mtd_ObtenerDireccionRepositorioLocalActualizaciones();

                    if (_Str_DireccionActT3 != "") // identificó que está en una red con repositorio, entonces intenta obtener la actualización..
                    {
                        MessageBox.Show("Se actualizará localmente a la versión " + _Str_Version + " para ingresar al sistema.", "Requerimiento", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        try
                        {
                        ...
                        {
                            // descarga de la página de conssa la actualización
                            this.Activated -= new EventHandler(Frm_Inicio_Activated);
                            MessageBox.Show("*Debe descargar la versión " + _Str_Version + " para ingresar al sistema.", "Requerimiento", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    MessageBox.Show("Se va actualizar a la versión " + _Str_Version + " para ingresar al sistema.", "Requerimiento", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    catch
                    {
                        ...
                        if (File.Exists(_Str_Ejecutable_T3Dominio))
                        {
                            ...
                            {
                                MessageBox.Show("Lo sentimos, la versión que debe ser instalada (" + _Str_Version.Trim() + ") no puede ser encontrada. Por favor contacte al administrador del sistema.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                Cursor = Cursors.Default;
                            }
                        }
                        else
        ...
        /// <summary>
        /// Devuelve una cadena de texto con la dirección del repositorio local de actualizaciones.
        /// Esta cadena es un valor estático, y no se obtiene de la base de datos, debido a que se usa en modo "remoto".
        /// La rutina se podría mejorar abriendo una conexion a la base de datos local (en caso de estar disponible),
        /// y verificando si localmente está dispoible la actualizacion que solicita la BD remota.
        /// </summary>
        /// <returns></returns>
        private string _Mtd_ObtenerDireccionRepositorioLocalActualizaciones()
        {
            ...
            // identifica si está en una red con conocida con repositorio
