    ...
    public partial class Frm_Inf_IncVolVentas : Form
    {
        ...
        private void _Mtd_CargarGrupos()
        {
            ...
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TGRUPOIV.cidgrupincentivar, TGRUPOIV.cdescripcion " +
                                 ...
                                 "TCALINCVTAS ON TGRUPOIV.cgroupcomp = TCALINCVTAS.cgroupcomp AND TGRUPOIV.ccompany = TCALINCVTAS.ccompany AND " +
                                 "TGRUPOIV.cidgrupincentivar = TCALINCVTAS.cidgrupincentivar " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCVTAS.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCVTAS.cyearacco = '" + _Str_Valores[1] + "') AND (TCALINCVTAS.cmontacco = '" + _Str_Valores[0] + "') " +
        ...
        {
            string[] _Str_Valores = _Mtd_ExtraerMesAno(_Cmb_Mes.SelectedValue.ToString());
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TCALINCVTAS.cvendedor, TCALINCVTAS.cvendedor+' - '+ TUSER.cname, TUSER.cname " +
                                 "FROM TGRUPOIV INNER JOIN " +
                                 "TCALINCVTAS ON TGRUPOIV.cgroupcomp = TCALINCVTAS.cgroupcomp AND TGRUPOIV.ccompany = TCALINCVTAS.ccompany AND " +
                                 "TGRUPOIV.cidgrupincentivar = TCALINCVTAS.cidgrupincentivar INNER JOIN " +
                                 "TUSER ON TCALINCVTAS.cvendedor = TUSER.cuser " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCVTAS.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCVTAS.cyearacco = '" + _Str_Valores[1] + "') AND (TCALINCVTAS.cmontacco = '" + _Str_Valores[0] + "') AND (TGRUPOIV.cidgrupincentivar = '" + Convert.ToString(_Cmb_Grupo.SelectedValue).Trim() + "') " +
        ...
        private void Frm_Inf_IncVolVentas_Load(object sender, EventArgs e)
        {
        ...
        {
            _Rpt_Report.ServerReport.DisplayName = CLASES._Cls_Varios_Metodos._Mtd_NombreReportesExportacion("RPT_INC_VOL_VENTAS");
