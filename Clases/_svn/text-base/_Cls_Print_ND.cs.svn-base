...
namespace T3.Clases
{
    public class _Cls_Print_ND
    {
        ...
        public _Cls_Print_ND()
        {
            this._My_Documento.PrintPage += new PrintPageEventHandler(_My_Documento_PrintPage);
            _Bol_G_PasePrintDialog = false;
        }
        public _Cls_Print_ND(string _Pr_Str_cgroupcomp, string _Pr_Str_ccompany, string _Pr_Str_ND, PrintDialog _Pr_PrintDialog)
        {
        ...
        public string _Field_IdCliente
        {
            ...
            {
                string _Str_R = "";
                string _Str_Sql = "SELECT ccliente FROM TNOTADEBICC WHERE cgroupcomp='" + _Str_Gcgroupcomp + "' AND ccompany='" + _Str_Gccompany + "' AND cidnotadebitocc='" + _Str_G_ND + "'";
        ...
        public string _Field_Concepto
        {
            ...
            {
                string _Str_R = "";
                string _Str_Sql = "SELECT cdescripcion FROM TNOTADEBICC WHERE cgroupcomp='" + _Str_Gcgroupcomp + "' AND ccompany='" + _Str_Gccompany + "' AND cidnotadebitocc='" + this._Str_G_ND + "'";
                DataSet _Ds = Program._MyClsCnn._mtd_conexion._Mtd_RetornarDataset(_Str_Sql);
                if (_Ds.Tables[0].Rows.Count > 0)
                {
                    _Str_R = Convert.ToString(_Ds.Tables[0].Rows[0][0]).Trim();
        ...
        public double _Field_MontoSimp
        {
            ...
            {
                double _Dbl_R = 0;
                string _Str_Sql = "SELECT cmontototsi FROM TNOTADEBICC WHERE cgroupcomp='" + _Str_Gcgroupcomp + "' AND ccompany='" + _Str_Gccompany + "' AND cidnotadebitocc='" + this._Str_G_ND + "'";
                DataSet _Ds = Program._MyClsCnn._mtd_conexion._Mtd_RetornarDataset(_Str_Sql);
                if (_Ds.Tables[0].Rows.Count > 0)
                {
        ...
        public double _Field_MontoImp
        {
            ...
            {
                double _Dbl_R = 0;
                string _Str_Sql = "SELECT cimpuesto FROM TNOTADEBICC WHERE cgroupcomp='" + _Str_Gcgroupcomp + "' AND ccompany='" + _Str_Gccompany + "' AND cidnotadebitocc='" + this._Str_G_ND + "'";
                DataSet _Ds = Program._MyClsCnn._mtd_conexion._Mtd_RetornarDataset(_Str_Sql);
                if (_Ds.Tables[0].Rows.Count > 0)
                {
                    _Dbl_R = Convert.ToDouble(_Ds.Tables[0].Rows[0][0]);
        ...
        public double _Field_MontoTotal
        {
            ...
            {
                double _Dbl_R = 0;
                string _Str_Sql = "SELECT ctotaldocu FROM TNOTADEBICC WHERE cgroupcomp='" + _Str_Gcgroupcomp + "' AND ccompany='" + _Str_Gccompany + "' AND cidnotadebitocc='" + this._Str_G_ND + "'";
                DataSet _Ds = Program._MyClsCnn._mtd_conexion._Mtd_RetornarDataset(_Str_Sql);
                if (_Ds.Tables[0].Rows.Count > 0)
                {
                    _Dbl_R = Convert.ToDouble(_Ds.Tables[0].Rows[0][0]);
        ...
        public string _Field_Titulo
        {
            get
            {
                string _Str_R = "NOTA DE DEBITO N# " + _Str_G_ND;
                return _Str_R;
            }
        }
        private void _My_Documento_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            //if (_Str_Concepto.Length > 70)
            //{
            //    _Str_Concepto = _Str_Concepto.Substring(0, 70);
            //}
