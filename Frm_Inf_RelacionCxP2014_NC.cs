...
namespace T3
{
    public partial class Frm_Inf_RelacionCxP2014_NC : Form
    {
        #region Atributos
        ...
        /// <summary>Cuando es verdadero, muestra las notas de débito al proveedor.</summary>
        private bool gMostrarNDC = true;
        ...
        /// <param name="pMostrarNDP">Verdadero, muestra las notas de débito al proveedor.</param>
        public Frm_Inf_RelacionCxP2014_NC(bool pMostrarNDP)
        {
            InitializeComponent();

            gMostrarNDC = pMostrarNDP;
        ...
        private void mostrarNotasCreditoAlProveedor(string pGrupo, string pCompañia, string pTipo, bool pImpresa, string pDesde, string pHasta)
        {
            string sSQL;

            sSQL = "select cidnotacreditocxp, cfechanc, cglobal, cproveedor, c_nomb_comer, c_rif, cmontototsi, cbaseexcenta, cimpuesto, ctotaldocu from VST_TNOTACREDICP where cgroupcomp='" + pGrupo + "' and ccompany='" + pCompañia + "'";
            sSQL += " and cimpresa=" + (pImpresa ? "1" : "0") + " AND canulado=0 and cglobal=" + (pTipo != "nulo" ? pTipo : "1");
            sSQL += " and convert(datetime, convert(varchar(255), cfechanc, 103)) between '" + pDesde + "' and '" + pHasta + "'";
            ...
            oParametros[2] = new ReportParameter("CESTATUS", "1");

            reiniciarVisorReportes();

            _Rpt_VisorReportes.LocalReport.ReportEmbeddedResource = "T3.ReportServLocal.Rpt_Inf_CxP_NC.rdlc";
        ...
        /// <summary>Muestra las notas de crédito del proveedor.</summary>
        /// <param name="pCompañia">Compañía.</param>
        /// <param name="pProveedor">Tipo de proveedor.</param>
        /// <param name="pDesde">Fecha desde.</param>
        /// <param name="pHasta">Fecha hasta.</param>
        /// <param name="pCategoria">Categoría.</param>
        /// <param name="pTipo">Tipo de proveedor.</param>
        private void mostrarNotasCreditoDelProveedor(string pCompañia, string pProveedor, string pDesde, string pHasta, string pCategoria, string pTipo)
        {
            ReportParameter[] oParametros = new ReportParameter[8];

            DataSet oRIF = Program._MyClsCnn._mtd_conexion._Mtd_RetornarDataset("select crif from TCOMPANY where ccompany='" + Frm_Padre._Str_Comp + "';");

            oParametros[0] = new ReportParameter("CCOMPANY", pCompañia);
            oParametros[1] = new ReportParameter("CNOMCOMP", CLASES._Cls_Varios_Metodos._Mtd_NombComp(Frm_Padre._Str_Comp));
            oParametros[2] = new ReportParameter("CPROVEEDOR", (pProveedor != "nulo" ? pProveedor : ""));
            oParametros[3] = new ReportParameter("CFECHADESDE", pDesde);
            oParametros[4] = new ReportParameter("CFECHAHASTA", pHasta);
            oParametros[5] = new ReportParameter("CCATPROVEEDOR", (pCategoria != "nulo" ? pCategoria : ""));
            oParametros[6] = new ReportParameter("CGLOBAL", pTipo);
            ...
            _Rpt_VisorReportes.ServerReport.ReportPath = CLASES._Cls_Conexion._Str_ReportPath + "Rpt_Inf_CxP_NCP";
        ...
        private void _Ctrl_Multifiltro_BotonConsultarClick(List<FiltroGrid> pFiltros)
        {
            ...
            if (gMostrarNDC)
            {
                Text = "Reporte de notas de crédito al proveedor";

                mostrarNotasCreditoAlProveedor
                (
            ...
            {
                Text = "Reporte de notas de crédito del proveedor";

                mostrarNotasCreditoDelProveedor
                (
                    Frm_Padre._Str_Comp,
                    sProveedor,
                    sDesde,
