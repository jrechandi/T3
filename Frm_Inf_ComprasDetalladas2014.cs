...
namespace T3
{
    public partial class Frm_Inf_ComprasDetalladas2014 : Form
    {
        ...
        private void _Ctrl_Multifiltro_FiltroSeleccionado(FiltroCombo pFiltro, List<FiltroGrid> pFiltrosGrid)
        {
            switch (pFiltro.Nombre)
            {
                case "cotros":
                    ...
                    _Ctrl_Multifiltro.agregarEstado("RESUMIDO", "1");
                    _Ctrl_Multifiltro.agregarEstado("DETALLADO", "0");
        ...
        {
            bool bImpuesto = false, bResumido = true;
            ...
            foreach (FiltroGrid oFiltro in pFiltros)
            {
                ...
                {
                    bResumido = (oFiltro.Valor == "1") ? true : false;
                }
            }

            if (bResumido)
            {
                sSQL = "select cidnotrecepc, cnumdocu, cempaquestotal, cunidadestotal, cmontosi, cmontoimp, cproveedor, c_nomb_comer, ccompany, cfechanotrecep from VST_NOTARECEPC";
                
            }
            else
            {
                sSQL = "select cproducto, produc_descrip, produc_descrip_2, cempaques, cunidades, cmontosi, cmontoimp, cproveedor, c_nomb_comer, ccompany from VST_COMPRAPRODUCTOS";                
            }
            ...
            _Rpt_VisorReportes.LocalReport.ReportEmbeddedResource = "T3.ReportServLocal." + ((bResumido == true) ? "Rpt_ListCprProvDet.rdlc" : "Rpt_ListCprProductos.rdlc");
