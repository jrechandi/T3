    ...
    public partial class Frm_Inf_IncMF : Form
    {
        ...
        private void _Mtd_CargarGrupos()
        {
            ...
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TGRUPOIV.cidgrupincentivar, TGRUPOIV.cdescripcion " +
                                 ...
                                 "TCALINCMF ON TGRUPOIV.cidgrupincentivar = TCALINCMF.cidgrupincentivar AND " +
                                 "TGRUPOIV.cgroupcomp = TCALINCMF.cgroupcomp AND TGRUPOIV.ccompany = TCALINCMF.ccompany " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCMF.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCMF.cano = '" + _Str_Valores[1] + "') AND (TCALINCMF.cmes = '" + _Str_Valores[0] + "') " +
        ...
        {
            string[] _Str_Valores = _Mtd_ExtraerMesAno(_Cmb_Mes.SelectedValue.ToString());
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TCALINCMF.cvendedor, TCALINCMF.cvendedor+' - '+ TUSER.cname, TUSER.cname " +
                                 "FROM TGRUPOIV INNER JOIN " +
                                 "TCALINCMF ON TGRUPOIV.cidgrupincentivar = TCALINCMF.cidgrupincentivar AND " +
                                 "TGRUPOIV.cgroupcomp = TCALINCMF.cgroupcomp AND TGRUPOIV.ccompany = TCALINCMF.ccompany INNER JOIN " +
                                 "TUSER ON TCALINCMF.cvendedor = TUSER.cuser " +
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCMF.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCMF.cano = '" + _Str_Valores[1] + "') AND (TCALINCMF.cmes = '" + _Str_Valores[0] + "') AND (TGRUPOIV.cidgrupincentivar = '" + Convert.ToString(_Cmb_Grupo.SelectedValue).Trim() + "') " +
        ...
        private void Frm_Inf_IncMF_Load(object sender, EventArgs e)
        {
        ...
        {
            _Rpt_Report.ServerReport.DisplayName = CLASES._Cls_Varios_Metodos._Mtd_NombreReportesExportacion("RPT_INC_MF");
