...
namespace T3
{
    public partial class Frm_Inf_OrdenPago2014 : Form
    {
        #region Atributos
        ...
        private void Frm_Inf_OrdenPago2014_Resize(object sender, EventArgs e)
        {
        ...
        private void _Ctrl_Multifiltro_FiltroSeleccionado(FiltroCombo pFiltro, List<FiltroGrid> pFiltrosGrid)
        {
            ...
            switch (pFiltro.Nombre)
            {
                ...
                case "cestatus":
                    ...
                    _Ctrl_Multifiltro.eliminarEstados();
                    _Ctrl_Multifiltro.agregarEstado("CANCELADAS", "0");
                    _Ctrl_Multifiltro.agregarEstado("PENDIENTES", "1");
                    _Ctrl_Multifiltro.mostrarFiltro(_Enu_MultifiltroTipos.MultifiltroTipoCombo);
        ...
        {
            string sProveedor = "nulo", sDesde = "", sHasta = "", sAño = "", sMes = "", sCancelado = "1", sCategoria="nulo", sGlobal="9";
            ...
            foreach (FiltroGrid oFiltro in pFiltros)
            {
                ...
                if (oFiltro.Nombre == "cglobal")
                {
                    sGlobal = oFiltro.Valor.Trim();
                }

                if (oFiltro.Nombre == "ccancelado")
                {
                    sCancelado = oFiltro.Valor.Trim();
                }
            }
            
            ReportParameter[] oParametros = new ReportParameter[10];

            oParametros[0] = new ReportParameter("CGROUPCOMP", Frm_Padre._Str_GroupComp.Trim());
            oParametros[1] = new ReportParameter("CCOMPANY", Frm_Padre._Str_Comp.Trim());
            oParametros[2] = new ReportParameter("CNOMBEMP", CLASES._Cls_Varios_Metodos._Mtd_NombComp(Frm_Padre._Str_Comp));
            oParametros[3] = new ReportParameter("CPROVEEDOR", sProveedor);
            oParametros[4] = new ReportParameter("CCATEGPROVE", sCategoria);
            oParametros[5] = new ReportParameter("CGLOBAL", sGlobal);
            oParametros[6] = new ReportParameter("CFECHAI", sDesde);
            oParametros[7] = new ReportParameter("CFECHAF", sHasta);
            oParametros[8] = new ReportParameter("CCANCELADO", sCancelado);
            oParametros[9] = new ReportParameter("CIMPRESO", sCancelado);

            _Rpt_VisorReportes.ServerReport.ReportServerUrl = new Uri(CLASES._Cls_Conexion._Str_ReportServerUrl);
            _Rpt_VisorReportes.ServerReport.ReportPath = CLASES._Cls_Conexion._Str_ReportPath + "Rpt_Inf_ListOrdenPago";
