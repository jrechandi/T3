...
namespace T3
{
    public partial class Frm_Inf_IncDistribucion : Form
    {
        CLASES._Cls_Varios_Metodos _Cls_VariosMetodos = new T3.CLASES._Cls_Varios_Metodos(true);
        public Frm_Inf_IncDistribucion()
        {
            InitializeComponent();
            _Rpt_Report.ServerReport.ReportServerUrl = new Uri(CLASES._Cls_Conexion._Str_ReportServerUrl);
            _Rpt_Report.ServerReport.ReportPath = CLASES._Cls_Conexion._Str_ReportPath + "Rpt_Inf_IncDistribucion"; 
        ...
        private void _Mtd_CargarGrupos()
        {
            ...
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TGRUPOIV.cidgrupincentivar, TGRUPOIV.cdescripcion " +
                            ...
                            "TCALINCDISTRIBU ON TGRUPOIV.cgroupcomp = TCALINCDISTRIBU.cgroupcomp AND TGRUPOIV.ccompany = TCALINCDISTRIBU.ccompany AND " +
                            "TGRUPOIV.cidgrupincentivar = TCALINCDISTRIBU.cidgrupincentivar " +
                            "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCDISTRIBU.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCDISTRIBU.cano = '" + _Str_Valores[1] + "') AND (TCALINCDISTRIBU.cmes = '" + _Str_Valores[0] + "') " +
        ...
        {
            string[] _Str_Valores = _Mtd_ExtraerMesAno(_Cmb_Mes.SelectedValue.ToString());
            string _Str_Cadena = "SELECT DISTINCT TOP 100 PERCENT TCALINCDISTRIBU.cvendedor, TCALINCDISTRIBU.cvendedor+' - '+ TUSER.cname, TUSER.cname "+
                                 "FROM TGRUPOIV INNER JOIN "+
                                 "TCALINCDISTRIBU ON TGRUPOIV.cgroupcomp = TCALINCDISTRIBU.cgroupcomp AND TGRUPOIV.ccompany = TCALINCDISTRIBU.ccompany AND "+
                                 "TGRUPOIV.cidgrupincentivar = TCALINCDISTRIBU.cidgrupincentivar INNER JOIN "+
                                 "TUSER ON TCALINCDISTRIBU.cvendedor = TUSER.cuser "+
                                 "WHERE (TGRUPOIV.cgroupcomp = '" + Frm_Padre._Str_GroupComp + "') AND (TGRUPOIV.ccompany = '" + Frm_Padre._Str_Comp + "') AND (TCALINCDISTRIBU.ctipogeneracion = '" + Convert.ToString(_Cmb_Generacion.SelectedValue).Trim() + "') AND (TCALINCDISTRIBU.cano = '" + _Str_Valores[1] + "') AND (TCALINCDISTRIBU.cmes = '" + _Str_Valores[0] + "') AND (TGRUPOIV.cidgrupincentivar = '" + Convert.ToString(_Cmb_Grupo.SelectedValue).Trim() + "') " +
        ...
        private void _Mtd_Busqueda()
        {
            ...
            parm[1] = new ReportParameter("CIDGRUPINCENTIVAR", _Str_GrupoInc);
            parm[2] = new ReportParameter("CMONTACCO", _Str_Valores[0]);
            parm[3] = new ReportParameter("CYEARACCO", _Str_Valores[1]);
            parm[4] = new ReportParameter("CVENDEDOR", _Str_Usuario);
        ...
        private void Frm_Inf_IncDistribucion_Load(object sender, EventArgs e)
        {
        ...
        {
            _Rpt_Report.ServerReport.DisplayName = CLASES._Cls_Varios_Metodos._Mtd_NombreReportesExportacion("RPT_INC_DISTRIBUCION");
